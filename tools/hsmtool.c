...
static void show_usage(const char *progname)
{
	...
	printf("	- printcodexsecret <path/to/hsm_secret> <id>\n");
	printf("	- printemergencyrecover <path/to/emergency.recover>\n");
	printf("	- printnodeid <path/to/hsm_secret>\n");
	exit(0);
}
...
int main(int argc, char *argv[])
{
	...
	if (streq(method, "decrypt")) {
		...
		if (argc < 3)
			show_usage(argv[0]);
		make_rune(argv[2]);
	} else if(streq(method, "printcodexsecret")) {
		if (argc < 4)
			show_usage(argv[0]);
		print_codexsecret(argv[2], argv[3]);
	} else if(streq(method, "printemergencyrecover")) {
		if (argc < 3)
			show_usage(argv[0]);
		print_emergencyrecover(argv[2]);
	} else if (streq(method, "printnodeid")) {
		if (argc < 3)
			show_usage(argv[0]);
