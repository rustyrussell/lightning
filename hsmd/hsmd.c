...
/* This is the common pattern for the tail of each handler. */
				 ...
				 const u8 *msg_out TAKES)
{
	 ...
	 * a simple request/response system like this, as we don't do any real
	 * buffering: if they don't read from their end, eventually we won't
	 * read any more commands from them once the pipe fills. */
 ...
 * will be generated mixing in the dbid and the peer node_id. */
				...
				const u8 *msg_in)
{
	 ...
	 * definitions in hsm_client_wire_csv.  The format of those files is
						       ...
						       const u8 *msg_in)
{
	...
	/*~ hsm_client_wire_csv marks the secret field here optional, so it only
...
/*~ This routine checks that a client is allowed to call the hander. */
