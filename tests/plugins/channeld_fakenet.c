...
static void make_privkey(size_t idx, struct privkey *pk)
{
	...
	if (idx & 1) {
		...
		sprintf((char *)&hsm_secret, "lightning-%zu", idx >> 1);
					  ...
					  struct gossmap_node **me)
{
	...
	for (size_t i = 0; i < tal_count(info->cached_node_idx); i++) {
		...
		if (!ecdh_maybe_blinding(&op->ephemeralkey, blinding, shared_secret))
			abort();
	...
	switch (rs->nextcase) {
	case ONION_END:
		*next_onion_packet = NULL;
		break;
	case ONION_FORWARD:
		*next_onion_packet = serialize_onionpacket(ctx, rs->next);
		break;
	}
...
found_next:
	...
	if (payload->blinding) {
		struct sha256 sha;
		blinding_hash_e_and_ss(payload->blinding,
				       &payload->blinding_ss,
				       &sha);
		next_blinding = tal(tmpctx, struct pubkey);
		blinding_next_pubkey(payload->blinding, &sha,
				     next_blinding);
	} else
		next_blinding = NULL;
	...
	dfwd->blinding = tal_steal(dfwd, next_blinding);
	dfwd->expected = next;
