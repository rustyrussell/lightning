...
@unittest.skipIf(not config['COMPAT'], "Needs COMPAT_V070 to read store v3")
def test_gossip_store_load_v3(node_factory):
...
def test_gossip_store_load(node_factory):
    ...
    l1 = node_factory.get_node(start=False)
    with open(os.path.join(l1.daemon.lightning_dir, 'gossip_store'), 'wb') as f:
        f.write(bytearray.fromhex("05"  # GOSSIP_VERSION
                                  "000001b0"  # len
                                  "dc5bef89"  # csum
                                  "5b8d9b44"  # timestamp
