...
def test_restorefrompeer(node_factory, bitcoind):
    ...
    # This will NOT save peer storage, since it's sent before establishment!  But reconnecting will.
    ...
    l2.daemon.wait_for_logs(['peer_out WIRE_PEER_STORAGE',
                             'peer_in WIRE_PEER_STORAGE'])
    assert not l1.daemon.is_in_log('peer_in WIRE_PEER_STORAGE_RETRIEVAL')
    assert not l2.daemon.is_in_log('peer_in WIRE_PEER_STORAGE_RETRIEVAL')
