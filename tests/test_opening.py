...
def test_zeroconf_open(bitcoind, node_factory):
    ...
    assert ret['channel_type'] == {'bits': [12, 22, 50], 'names': ['static_remotekey/even', 'anchors_zero_fee_htlc_tx/even', 'zeroconf/even']}
    assert only_one(l1.rpc.listpeerchannels(l2.info['id'])['channels'])['channel_type'] == {'bits': [12, 22, 50], 'names': ['static_remotekey/even', 'anchors_zero_fee_htlc_tx/even', 'zeroconf/even']}
...
def test_zeroconf_public(bitcoind, node_factory, chainparams):
    ...
    # Channel is "proposed"
    chan_val = 968888000 if chainparams['elements'] else 970073000
    l1_mvts = [
