...
def test_pay_direct(node_factory, bitcoind):
    ...
    c01 = l0.fund_channel(l1, 10**7, wait_for_active=False)
    l1.rpc.connect(l3.info['id'], 'localhost', l3.port)
    c13 = l1.fund_channel(l3, 10**7, wait_for_active=False)
    # Indirect route l0->l1->l2->l3
    l1.rpc.connect(l2.info['id'], 'localhost', l2.port)
    c12 = l1.fund_channel(l2, 10**7, wait_for_active=False)
    l2.rpc.connect(l3.info['id'], 'localhost', l3.port)
    c23 = l2.fund_channel(l3, 10**7, wait_for_active=False)

    # Let channels lock in.
    bitcoind.generate_block(5)

    # Make sure l0 knows all the channels.
    # Without DEVELOPER, channel lockin can take 30 seconds to detect,
    # and gossip 2 minutes to propagate
    wait_for(lambda: [l0.is_channel_active(c) for c in [c01, c13, c12, c23]] == [True, True, True, True], timeout=180)
