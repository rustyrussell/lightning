...
{
	return has_announce_depth(channel)
...
#define check_channel_gossip(channel)  check_channel_gossip_((channel), __LINE__)
static void check_channel_gossip_(const struct channel *channel, int line)
{
	struct channel_gossip *cg = channel->channel_gossip;
	bool enabled;

	log_debug(channel->log, "checking channel gossip (%u)...", line);
...
#define set_gossip_state(channel, state) \
	set_gossip_state_((channel), (state), __LINE__)
static void set_gossip_state_(struct channel *channel,
			      enum channel_gossip_state state, int line)
{
	struct channel_gossip *cg = channel->channel_gossip;
