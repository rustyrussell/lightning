...
static const char *json_param_route(const tal_t *ctx,
				    struct command *cmd,
				    const char *buffer,
				    ...
				    struct route_hop **route)
{
	...
	n_hops = 0;
	*route = tal_arr(ctx, struct route_hop, n_hops);

	for (const jsmntok_t *t = routetok + 1; t < end; t = json_next(t)) {
		const jsmntok_t *amttok, *idtok, *delaytok, *chantok;

		if (t->type != JSMN_OBJECT)
			return tal_fmt(cmd, "Non-object as array element '%.*s'",
		...
		if (!amttok || !idtok || !delaytok || !chantok) {
			return tal_fmt(cmd, "Need msatoshi, id, delay"
				       " and channel in '%.*s'",
				       t->end - t->start, buffer + t->start);
		}
		...
		if (!json_tok_number(buffer, amttok, &(*route)[n_hops].amount)) {
			return tal_fmt(cmd, "invalid amount '%.*s'",
				       t->end - t->start, buffer + t->start);
		}

		if (!json_tok_short_channel_id(buffer, chantok,
					       &(*route)[n_hops].channel_id)) {
			return tal_fmt(cmd, "invalid channel '%.*s'",
				       t->end - t->start, buffer + t->start);
		}
		...
		if (!json_tok_number(buffer, delaytok, &(*route)[n_hops].delay)) {
			return tal_fmt(cmd, "invalid delay '%.*s'",
				       t->end - t->start, buffer + t->start);
		}
		n_hops++;
	}
...
#define JSON_PARAM_ROUTE(name, ptr) \
	JSON_PARAM_ALLOCS_(name, ptr, json_param_route)
			 ...
			 const char *buffer, const jsmntok_t *params)
{
	...
	if (!json_params(tmpctx, cmd, buffer, params,
			 JSON_PARAM_ROUTE("route", &route),
			 JSON_PARAM_SHA256("payment_hash", &rhash),
			 NULL)) {
		return;
	}
			     ...
			     const jsmntok_t *params)
{
	...
	if (!json_params(tmpctx, cmd, buffer, params,
			 JSON_PARAM_SHA256("payment_hash", &rhash),
			 JSON_PARAM_OPT_U32("timeout", &timeout),
			 NULL))
		return;
