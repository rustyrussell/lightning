			       ...
			       char **fail)
{
	 ...
	 * doesn't matter what engine we use (short will have 93 bytes of payload) */
	csum_engine = &initial_engine_csum[maxlen > 93];
	...
	if (!pull_chars(&threshold_char, 1, &codex_datastr, &maxlen)
	    || !pull_chars(parts->id, ARRAY_SIZE(parts->id), &codex_datastr, &maxlen)
	...
	/* Is payload too long for long checksum? */
	if (maxlen > 103) {
