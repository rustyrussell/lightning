...
/* Set in init */
static struct pay_plugin *pay_plugin;
...
static void paynote(struct payment *p, const char *fmt, ...)
	PRINTF_FMT(2,3);
...
static void amount_msat_accumulate_(struct amount_msat *dst,
...
static void amount_msat_reduce_(struct amount_msat *dst,
				    struct amount_msat src,
				    const char *dstname,
				    const char *srcname)
{
...
/* We're not using these flows after all: chean up chan_extra_map */
...
static struct pay_flow **get_payflows(struct payment *p,
				      struct amount_msat amount,
					 ...
					 bool first_time)
{
	...
	pay_flows = get_payflows(p, p->amount, remaining, first_time);
	if (!pay_flows)
