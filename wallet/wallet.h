 ...
 * @wallet: wallet to load from
 * @chan: load HTLCs associated with this channel
 * @htlcs_in: htlc_in_map already populted by wallet_htlcs_load_in_for_channel
 * @htlcs_out: htlc_out_map to store loaded htlc_out in.
 *
 * We populate htlc_out->in by looking up in htlcs_in.  It's possible that it's
 * still NULL, since we can have outgoing HTLCs outlive their corresponding
 * incoming.
 */
