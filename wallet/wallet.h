 ...
 * @addrtypes: (in) addess types we will publish for this
 *
 * Returns -1 on error (key exhaustion).
 */
s64 wallet_get_newindex(struct lightningd *ld, enum addrtype addrtypes);

/**
 * wallet_get_addrtypes - get the address types for this key.
 * @wallet: (in) wallet
 * @keyidx: what address types we've published.
 */
enum addrtype wallet_get_addrtypes(struct wallet *w, u64 keyidx);
