...
static struct peer *wallet_peer_load(struct wallet *w, const u64 dbid)
{
	...
	struct peer *peer;
	struct pubkey id;
	struct wireaddr_or_sockname *addrp, addr;
	...
	if (addrstr) {
		addrp = &addr;
		if (!parse_wireaddr_or_sockname((const char*)addrstr, addrp, DEFAULT_PORT, NULL)) {
			db_stmt_done(stmt);
			return NULL;
		}
	} else
...
void wallet_channel_insert(struct wallet *w, struct channel *chan)
{
	...
	if (chan->peer->dbid == 0) {
		...
		if (!chan->peer->addr.is_sockname
		    && chan->peer->addr.u.wireaddr.type == ADDR_TYPE_PADDING)
			sqlite3_bind_null(stmt, 2);
		else
			sqlite3_bind_text(stmt, 2,
					  type_to_string(tmpctx, struct wireaddr_or_sockname, &chan->peer->addr),
					  -1, SQLITE_TRANSIENT);
		db_exec_prepared(w->db, stmt);
