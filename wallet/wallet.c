...
struct rune_blacklist *wallet_get_runes_blacklist(const tal_t *ctx, struct wallet *wallet)
{
	...
	stmt = db_prepare_v2(wallet->db, SQL("SELECT start, end FROM runes_blacklist"));
	db_query_prepared(stmt);

	while (db_step(stmt)) {
		struct rune_blacklist b;
		b.start = db_col_u64(stmt, "start");
		b.end = db_col_u64(stmt, "end");
